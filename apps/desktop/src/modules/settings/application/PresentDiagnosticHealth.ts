...
const STATUS_WEIGHT: Readonly<Record<DiagnosticCheckStatus, number>> =
  Object.freeze({
    passed: 0,
    warning: 1,
    failed: 2,
...
function worstStatus(
  ...
  return checks.reduce<DiagnosticCheckStatus>(
    (current, check) =>
      STATUS_WEIGHT[check.status] > STATUS_WEIGHT[current]
        ? check.status
        : current,
    "passed",
  );
...
function reportCopy(
  ...
  if (status === "healthy") {
    return {
      ...
      description:
        "Your words, settings, and backups are available. Nothing needs your attention.",
    };
  }
...
export function presentDiagnosticHealth(
  report: DiagnosticReport,
): DiagnosticHealthPresentation {
  ...
  const backupCheck = report.checks.find(
    (check) => check.id === "backup-availability",
  );
  const dataChecks = report.checks.filter(
    (check) => check.id !== "backup-availability",
  );
