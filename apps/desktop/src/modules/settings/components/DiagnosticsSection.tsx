...
function outcomeCopy(status: DiagnosticOverallStatus) {
  if (status === "healthy") {
    return {
      ...
      description:
        "Your words, settings, and backups are available. No action is needed.",
    };
  }
  ...
  if (status === "attention") {
    return {
      ...
      description:
        "Your data is still available. Review the suggested action below.",
    };
  }
  ...
  return {
    ...
    description:
      "Keep the app open and follow the recovery guidance before making more changes.",
  };
}
...
export function DiagnosticsSection({
  ...
  const repairableIssues =
    report?.checks.filter((check) => check.repairable) ?? [];
  ...
  async function handleSafeMaintenance() {
    ...
    try {
      ...
      setError(
        cause instanceof Error
          ? cause.message
          : "The safe repair could not be completed.",
      );
      setStatus("error");
    }
  }
    ...
    <div className="diagnostics-section">
      <div className="diagnostics-toolbar">
        <div>
          ...
          <p>
            This check reads local information only. It does not change or
            upload anything.
          </p>
        </div>
        <Button
      ...
      {error === undefined ? null : (
        <div className="diagnostics-error" role="alert">
          ...
          <div>
            ...
            <p>
              Nothing was changed. Try again, or open the technical details for
              support.
            </p>
      ...
      {report === undefined ? (
        <div className="diagnostics-empty">
          ...
          <div>
            ...
            <p>
              Run a quick local check whenever something does not look right.
            </p>
          </div>
        </div>
      ) : (
          ...
          <dl className="diagnostics-summary-list">
            <div>
              ...
              <dd>
                {issueCount(report) === 0
                  ? "Ready"
                  : `${issueCount(report)} item(s) to review`}
              </dd>
            </div>
            <div>
              <dt>Backups</dt>
              <dd>
                {report.counts.retainedBackups > 0
                  ? "Available"
                  : "Not available"}
              </dd>
            </div>
          ...
          {nonRepairableFailures.length === 0 ? null : (
            <div className="diagnostics-guidance" role="alert">
              ...
              <div>
                ...
                <p>
                  This issue cannot be fixed automatically. Open Data & backups
                  and restore the newest backup that passes its check.
                </p>
              </div>
            </div>
          )}
          ...
          {repairableIssues.length === 0 ? null : (
            <section className="diagnostics-repair">
              <div>
                ...
                <p>
                  English Focus can restore the app storage it needs. Your
                  words, notes, settings, and backups are not deleted.
                </p>
              </div>
              <label>
                <input
            ...
            <div className="diagnostics-technical__body">
              <header>
                <div>
                  ...
                  <p>
                    English Focus {report.appVersion} · {report.checks.length}{" "}
                    checks
                  </p>
                </div>
                <Button
