...
describe("backup retention and scheduling", () => {
  ...
  it("calculates the next automatic backup delay instead of checking only at startup", () => {
    ...
    expect(automaticBackupDelayMs([], "daily", new Date("2026-07-15T13:00:00.000Z"))).toBe(0);
    expect(automaticBackupDelayMs([latest], "daily", new Date("2026-07-14T13:00:00.000Z"))).toBe(
      23 * 60 * 60 * 1_000
    );
    expect(automaticBackupDelayMs([latest], "weekly", new Date("2026-07-15T13:00:00.000Z"))).toBe(
      6 * 24 * 60 * 60 * 1_000 - 60 * 60 * 1_000
    );
    expect(automaticBackupDelayMs([], "manual", new Date("2026-07-15T13:00:00.000Z"))).toBe(
      undefined
    );
  });
  ...
  it("recovers a backup that was written before retention cleanup failed", () => {
    ...
    expect(findCreatedBackup([created], "automatic", createdAt)).toEqual(created);
