...
test("collection selection exposes bulk actions only when words are selected", async ({ page }) => {
  ...
  await expect(page.locator(".wvc-selection-bar").getByRole("button", { name: "Export" })).toBeEnabled();
  await expect(page.locator(".wvc-selection-bar").getByRole("button", { name: "Move" })).toBeEnabled();
