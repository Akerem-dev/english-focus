...
- failed backup inspection is shown as unavailable rather than as an empty backup folder;
- unavailable measurements are displayed as `Could not be checked` rather than a misleading zero;
- raw SQLite, parser, permission, and row-decoding messages remain outside the user-facing report;
- the user is directed to restart the app, run the check again, and validate a recent backup before changing local data.

A completed scan with zero findings remains visually distinct from a scan that did not finish.

## Verification boundary

The standard Quality workflow includes a Windows native-test job in addition to the existing JavaScript quality job. The native job compiles the Tauri crate and runs Rust tests on every pull request. When it fails, the focused Cargo output is retained briefly as a downloadable workflow artifact.
