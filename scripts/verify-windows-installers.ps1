...
$directory = Join-Path $root "release-artifacts/windows/$($config.version)"
...
if (-not $requireSignature) {
    Write-Host ""
    Write-Host "Unsigned CP27 release-candidate verification passed." -ForegroundColor Yellow
}
