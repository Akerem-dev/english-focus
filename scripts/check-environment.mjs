...
if (!existsSync(rootPackagePath) || !existsSync(desktopPackagePath)) {
  failures.push(
    "Run this command from the project root containing package.json and apps/desktop/package.json."
  );
...
if (!npmVersionText) {
  notices.push(
    "npm version could not be read from npm_config_user_agent. Confirm manually with npm -v."
  );
...
for (const fileName of ["package-lock.json", "npm-shrinkwrap.json"]) {
  ...
  if (matchedPattern) {
    failures.push(
      `${fileName} contains a forbidden internal registry reference: ${matchedPattern}`
    );
